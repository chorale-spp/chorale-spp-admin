...
APP.loadNotifications = async function(containerId, isAdminPortal) {
  // Wait for element — on mobile the section may render slightly late
  let el = document.getElementById(containerId);
  if (!el) {
    await new Promise(r => setTimeout(r, 300));
    el = document.getElementById(containerId);
  }
  if (!el) { console.warn('Notifications container not found:', containerId); return; }
  ...
  if (!uid) {
    el.innerHTML = '<p class="muted" style="padding:20px">Not signed in.</p>';
    return;
  }

  try {
    // Simple get — no compound query, no index needed
    ...
    if (!items.length) {
      el.innerHTML = `
        <div style="text-align:center;padding:48px 20px">
          ...
          <p class="muted" style="font-size:.88rem">No notifications yet.</p>
          <p style="font-size:.75rem;color:var(--text-light);margin-top:6px">You will be notified when an announcement is posted.</p>
        </div>`;
      return;
    }
    ...
    el.innerHTML = `
      ...
      ${items.map(n => {
        ...
        return `
          <div style="display:flex;gap:12px;padding:14px 0;border-bottom:1px solid rgba(0,0,0,.04);${isUnread ? 'background:rgba(184,151,58,0.03);border-radius:var(--radius);padding:14px 8px;' : ''}">
            <div style="width:38px;height:38px;border-radius:50%;background:rgba(184,151,58,0.1);display:flex;align-items:center;justify-content:center;flex-shrink:0;color:${t.colour}">
              <i class="fas ${t.icon}"></i>
            </div>
            <div style="flex:1;min-width:0">
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                <span style="font-weight:600;font-size:.84rem">${n.title}</span>
                ${isUnread ? '<span style="width:7px;height:7px;border-radius:50%;background:var(--gold);flex-shrink:0;display:inline-block"></span>' : ''}
              </div>
    ...
    el.innerHTML = `
      <div style="text-align:center;padding:30px">
        <i class="fas fa-exclamation-circle" style="font-size:2rem;color:var(--border);margin-bottom:10px;display:block"></i>
        <p class="muted" style="font-size:.82rem">Could not load notifications.</p>
        <button class="btn-ghost" style="margin-top:10px" onclick="APP.loadNotifications('${containerId}', ${isAdminPortal})">
...
// ── Get unread count ──
// Uses only a single where() to avoid composite index requirement
